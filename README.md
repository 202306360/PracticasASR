...
## 1ra Solución: Creación de la máquina de salto
...
Para permitir el acceso al jump-server desde nuestro pc hemos añadidola clave SSH en el "metadata" de Google Cloud. Una vez hecho esto y creadas las máquinas, ya podemos acceder al jump server desde nuestra consola de comandos. 
...
En este caso, la máquina http-server solo tendrá dirección IP interna, mientras que la jump-server tendrá interna y externa.
...
Para este apartado, mantendremos la misma configuración que en el ejercicio anterior, pero esta vez a mayores aplicamos las siguientes reglas:
...
En esta parte, buscamos que el contenido web esté cifrado también dentro del cloud. La última parte consiste en hacer trabajar al Nginx por el puerto 443 y que el balanceador vaya también por el HTTPS en lugar del HTTP. Los pasos a seguir son los siguientes:
...
2. Cambiar la configuración en el Backend del Load Balancer.
...
3. Modificar el Health Check del Load Balancer en el Backend.
...
## Parte 2.2: Proteger nuestra máquina de ataques SQL Injection, Cross-Site Scripting y restringir el tráfico solo a países de confianza de la UE implantando un WAF a nuestro balanceador
